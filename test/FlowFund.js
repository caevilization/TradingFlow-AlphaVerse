...
describe('FlowFund', function () {
  ...
  beforeEach(async function () {
    ...
    const TradingFlowToken =
      await ethers.getContractFactory('TradingFlowToken');
    token = await TradingFlowToken.deploy(
      'TradingFlow Token',
      'TFT',
      initialSupply
    );
  ...
  describe('Fund Manager NFTs', function () {
    ...
    it('Should update fund manager metadata', async function () {
      ...
      await fund
        .connect(addr1)
        .updateFundManagerMetadata(2, 'ipfs://metadata2');
  ...
  describe('Investment Operations', function () {
    ...
    it('Should distribute and claim dividends', async function () {
      ...
      expect(finalCarryBalance - initialCarryBalance).to.equal(
        (dividendAmount * BigInt(1500)) / BigInt(10000)
      ); // 15% carry fee
      expect(finalBalance - initialBalance).to.equal(
        (dividendAmount * BigInt(8500)) / BigInt(10000)
      ); // 85% to investor
    });
  });
});
